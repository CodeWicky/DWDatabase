...
/**
 ...
 当前模型转换支持基本属性转换，模型嵌套，容器属性容错转换，容器属性泛型转换
 （遍历时考虑是否采用CF中的遍历方式，待推敲）
 ...
 模型嵌套入库几个点：通过为Oject类型且NSType为Unknown识别模型嵌套，通过一个钩子方法提供表明定制，钩子方法提供数据库路径（考虑是否需要维护多个数据库文件），db获取的数据库信息中应该维护内联表名，应从最内层的嵌套开始存储，直到失败或者最外层为止，失败后不回滚，只取消上级的存储，改造插入方法返回值，带出Dw_id（思考是否要将Dw_id暴露给外界），update方法考虑是否能直接以条件模式做更新
