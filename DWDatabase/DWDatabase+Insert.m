    ...
    if (recursive) {
        ...
        if (record.finishOperationInChain) {
            ...
            if (fac.objMap.allKeys.count) {
                ...
                [maker.bindKeys addObjectsFromArray:fac.validKeys];
...
-(DWDatabaseResult *)insertSQLFactoryWithModel:(NSObject *)model dbName:(NSString *)dbName tableName:(NSString *)tblName insertChains:(DWDatabaseOperationChain *)insertChains recursive:(BOOL)recursive conditionMaker:(DWDatabaseConditionMaker *)maker {
    ...
    ///获取配置sql的相关参数
    [self handleInsertArgumentsWithPropertyInfos:infos dbName:dbName tblName:tblName model:model insertChains:insertChains recursive:recursive validKeysContainer:validKeys argumentsContaienr:args objMap:objMap subKeyArr:seperateKeys.lastObject];
                ...
                if (obj.type == DWPrefix_YYEncodingTypeObject && obj.nsType == DWPrefix_YYEncodingTypeNSUnknown) {
                    if (recursive) {
                        ...
                        if (existResult.success) {
                            ...
                            if (tblConf) {
                                subKeyToInsert = [self actualSubKeysIn:subKeyArr withPrefix:obj.name];
                            ...
                            if (!record.finishOperationInChain) {
                                ...
                                [self supplyModelSubKeys:value withPropertyInfo:obj propertyTblName:propertyTblName subKeyArr:subKeyArr subkeyToInsert:subKeyToInsert record:record conf:tblConf insertChains:insertChains validKeysContainer:validKeys argumentsContaienr:args objMap:objMap];
                            } else {
                                ...
                                ///开始更新二级字段
                                [self updateModelSubKeys:value propertyInfo:obj subKeyArr:subKeyArr subkeyToInsert:subKeyToInsert record:record conf:tblConf insertChains:insertChains validKeysContainer:validKeys argumentsContaienr:args objMap:objMap];
                            }
                        } else {
                            ///开始插入二级模型
                            [self insertNotExistModel:value propertyInfo:obj dbName:dbName tblName:tblName propertyTblName:propertyTblName subKeyArr:subKeyArr insertChains:insertChains inlineTblNameMap:inlineTblNameMap validKeysContainer:validKeys argumentsContaienr:args objMap:objMap];
                        }
                    }
                } else {
                    [validKeys addObject:propertyTblName];
...
-(void)supplyModelSubKeys:(NSObject *)model withPropertyInfo:(DWPrefix_YYClassPropertyInfo *)prop propertyTblName:(NSString *)propertyTblName subKeyArr:(NSArray <NSString *>*)subKeyArr subkeyToInsert:(NSArray <NSString *>*)subKeyToInsert record:(DWDatabaseOperationRecord *)record conf:(DWDatabaseConfiguration *)conf insertChains:(DWDatabaseOperationChain *)insertChains validKeysContainer:(NSMutableArray *)validKeys argumentsContaienr:(NSMutableArray *)args objMap:(NSMutableDictionary *)objMap {
    ///看一下有没有剩下的key需要补
    ...
    if (subKeyToInsertSet.count) {
        ///开始补key
        [self supplySubKeysWithModel:model propertyInfo:prop propertyTblName:propertyTblName subKeyArr:subKeyArr subKeyToInsertSet:subKeyToInsertSet record:record conf:conf insertChains:insertChains validKeysContainer:validKeys argumentsContaienr:args objMap:objMap];
    } else {
...
-(void)supplySubKeysWithModel:(NSObject *)model propertyInfo:(DWPrefix_YYClassPropertyInfo *)prop propertyTblName:(NSString *)propertyTblName subKeyArr:(NSArray *)subKeyArr subKeyToInsertSet:(NSSet *)subKeyToInsertSet record:(DWDatabaseOperationRecord *)record conf:(DWDatabaseConfiguration *)conf  insertChains:(DWDatabaseOperationChain *)insertChains validKeysContainer:(NSMutableArray *)validKeys argumentsContaienr:(NSMutableArray *)args objMap:(NSMutableDictionary *)objMap {
    ...
    NSMutableArray * subKeyRecursiveToInsert = [NSMutableArray arrayWithCapacity:0];
    [subKeyToInsert enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [subKeyRecursiveToInsert addObjectsFromArray:[self subKeysIn:subKeyArr withPrefix:prop.name actualSubKey:obj]];
    }];
    DWDatabaseConditionMaker * maker = [DWDatabaseConditionMaker new];
    maker.bindKeysWithArray(subKeyRecursiveToInsert);
    ...
    if (result.success) {
        ...
        objMap[prop.name] = result.result;
    } else {
...
-(void)updateModelSubKeys:(NSObject *)model propertyInfo:(DWPrefix_YYClassPropertyInfo *)prop subKeyArr:(NSArray *)subKeyArr subkeyToInsert:(NSArray <NSString *>*)subKeyToInsert record:(DWDatabaseOperationRecord *)record conf:(DWDatabaseConfiguration *)conf  insertChains:(DWDatabaseOperationChain *)insertChains validKeysContainer:(NSMutableArray *)validKeys argumentsContaienr:(NSMutableArray *)args objMap:(NSMutableDictionary *)objMap {
    ...
    if (subKeyToInsertSet.count) {
        ...
        DWDatabaseConditionMaker * maker = [DWDatabaseConditionMaker new];
        maker.bindKeysWithArray(subKeyRecursiveToInsert);
        DWDatabaseResult * result = [self dw_updateTableWithModel:model dbName:conf.dbName tableName:conf.tableName inQueue:conf.dbQueue updateChains:insertChains recursive:YES updateObjectID:NO conditionMaker:maker];
        if (!result.success) {
            ///如果插入失败了，再将刚才因此添加的key移除
            [record.operatedKeys minusSet:subKeyToInsertSet];
        }
...
-(void)insertNotExistModel:(NSObject *)model propertyInfo:(DWPrefix_YYClassPropertyInfo *)prop dbName:(NSString *)dbName tblName:(NSString *)tblName propertyTblName:(NSString *)propertyTblName subKeyArr:(NSArray *)subKeyArr insertChains:(DWDatabaseOperationChain *)insertChains inlineTblNameMap:(NSDictionary *)inlineTblNameMap validKeysContainer:(NSMutableArray *)validKeys argumentsContaienr:(NSMutableArray *)args objMap:(NSMutableDictionary *)objMap  {
    ///此处取嵌套模型对应地表名
    ...
    if (inlineTblName.length) {
        ...
        if (dbConf && [self createTableWithClass:prop.cls tableName:inlineTblName configuration:dbConf].success) {
            ...
            if (tblConf) {
                ///插入
                NSArray * subKeyToInsert = [self subKeysIn:subKeyArr withPrefix:prop.name];
                DWDatabaseConditionHandler condition = nil;
                if (subKeyToInsert.count) {
                    condition = ^(DWDatabaseConditionMaker * maker) {
                        maker.bindKeysWithArray(subKeyToInsert);
                    };
                }
